...
void gen_boxes()
{
	...
	ofstream out_file(&(("input/input_" + to_string(input_size) + ".txt").c_str())[0]);
	if (out_file.is_open())
	{
		input_boxes = new vector<box*>();
...
void read_input(int size_idx)
{
	ifstream in_file(&(("input/input_" + to_string(size_idx) + ".txt").c_str())[0]);
	if (in_file.is_open())
    {
    	int i;
    	std::string line;
...
void store_result(int size_idx, int sol, double elapse_time)
{
	ofstream out_file(&(("output/output_" + to_string(size_idx) + "_" + to_string(max_num_thread) + ".txt").c_str())[0]);
	if (out_file.is_open())
	{
		out_file << "solution: " << sol << "\n";
		out_file << "time: " << elapse_time << " (s)\n";
		out_file.close();
	}
}
...
int main(int argc, char **argv)
{
	...
	if (argc != 2)
	{
		cout << "Pass argument error!!! \n" ;
		cout << "### ./a.out <num_of_thread>" << endl;
		return 0;
	}

	max_num_thread = atoi(argv[1]);
	cout << "Num of thread: " << max_num_thread << endl;
	...
	for (i = 25; i <= 400;  i *= 2)
	{
		...
		store_result(i, solution, elapse);
		free_mem();
		cout << "======== Completed one input " << i << "========" << endl;
	}
	return 0;
}
...
void free_mem()
{
	int i;
	// free input_boxes
	for (i = 0; i < input_boxes->size(); i++)
	{
		delete input_boxes->at(i);
...
#pragma omp parallel private(father, mother, child)
		{
			vector<individual*> *private_population = new vector<individual*>();
#pragma omp for schedule(static) nowait
			for (j = 0; j < MAX_POPULATION; j++)
			{
				...
				mutation(child);
				calc_fitness(child);
				private_population->push_back(child);
			}
#pragma omp critical
			{
				new_population->insert(new_population->end(), private_population->begin(), private_population->end());
			}
		}
...
void mutation(individual *idv)
{
	...
	BPS_gene bps_gene_tmp = *idv->chrom->BPS->at(i);
	*idv->chrom->BPS->at(i) = *idv->chrom->BPS->at(j);
	*idv->chrom->BPS->at(j) = bps_gene_tmp;

	// swap i-th idv's BO-gene for j-th idv's BO-gene
	BO_gene bo_gene_tmp = *idv->chrom->VBO->at(i);
	*idv->chrom->VBO->at(i) = *idv->chrom->VBO->at(j);
	*idv->chrom->VBO->at(j) = bo_gene_tmp;
}
...
individual *random_chromosome()
{
	...
	for (i = 0; i < input_size; i++)
	{
		BPS_gene *bps_gene = new BPS_gene(input_boxes->at(arr[i])->id);
		BO_gene *bo_gene = new BO_gene();
...
int placement(chromosome *chrom)
{
	...
	for (i = 0; i < input_size; i++)
	{
		...
		for (j = 0; j < bins->size(); j++)
		{
			...
			EMS_idx = dftrc_2(input_boxes->at(chrom->BPS->at(i)->box_id), bins->at(j));
...
vector<EMS*> diff_process(EMS *ems, BPS_gene *box) { // chua clear ems
	...
	int x4 = x3 + input_boxes->at(box->box_id)->width;
	int y4 = y3 + input_boxes->at(box->box_id)->height;
	int z4 = z3 + input_boxes->at(box->box_id)->depth;
...
vector<point*> find_points(BPS_gene *box) {
	...
	point *RBF = new point(x + input_boxes->at(box->box_id)->width, y, z); //Point Right Bot Front
	point *LTF = new point(x, y + input_boxes->at(box->box_id)->height, z);//Point Left Top Front
	point *LBB = new point(x, y, z + input_boxes->at(box->box_id)->depth);//Point Left Bot Back
	vector<point*> points;
...
bin::bin()
{
	...
	this->flag = false;
	EMS *temp_ems = new EMS(new point(0, 0, 0), new point(bin_width, bin_height, bin_depth));
...
{
	int i;
	for (i = 0; i < this->EMSs->size(); i++)
	{
		delete this->EMSs->at(i);
		this->EMSs->erase(this->EMSs->begin() + i);
	}
	this->EMSs->clear();
	delete this->EMSs;
}
...
chromosome::~chromosome()
{
	...
	for (i = 0; i < input_size; i++)
	{
		this->BPS->at(i)->dispose();
		delete this->BPS->at(i);
		delete this->VBO->at(i);
	}
	this->BPS->clear();
	this->VBO->clear();
	delete this->BPS;
